...
mod flamegraph {
  ...
  impl Report {
    /// `flamegraph` will write an svg flamegraph into `writer` **only available with `flamegraph`
    /// feature**
    ...
    pub fn flamegraph_with_options<W>(&self, writer: W, options: &mut flamegraph::Options) -> Result<()>
    where
      W: std::io::Write,
    {
...
mod protobuf {
    ...
    pub fn pprof(&self) -> crate::Result<protos::Profile> {
      ...
      for (key, count) in self.data.iter() {
        ...
        let sample = protos::Sample {
          location_id: locs,
          value: vec![*count as i64, *count as i64 * 1_000_000_000 / self.timing.frequency as i64],
  ...
  mod tests {
    ...
    fn report(data: HashMap<Frames, isize>, frequency: i32) -> Report {
      Report {
        data,
        timing: ReportTiming {
          frequency,
    ...
    #[test]
    fn pprof_empty_report_has_required_metadata() {
      let profile = report(HashMap::new(), 100).pprof().unwrap();

      assert!(profile.sample.is_empty());
      assert_eq!(string_at(&profile, 0), "");
      assert_eq!(profile.sample_type.len(), 2);
      assert_eq!(string_at(&profile, profile.sample_type[0].ty), SAMPLES);
      assert_eq!(string_at(&profile, profile.sample_type[0].unit), COUNT);
      assert_eq!(string_at(&profile, profile.sample_type[1].ty), CPU);
      assert_eq!(string_at(&profile, profile.sample_type[1].unit), NANOSECONDS);
      assert_eq!(profile.period, 10_000_000);
      assert_eq!(profile.duration_nanos, 250_000_000);
    }

    #[test]
    fn pprof_report_maps_thread_label_and_sample_values() {
      let symbol = Symbol {
        name:     Some(b"sampled_function".to_vec()),
        addr:     None,
        lineno:   Some(42),
      ...
      let data = HashMap::from([(frames, 3)]);

      let profile = report(data, 100).pprof().unwrap();

      assert_eq!(profile.sample.len(), 1);
      let sample = &profile.sample[0];
      assert_eq!(sample.value, vec![3, 30_000_000]);
      assert_eq!(sample.location_id, vec![1]);
      assert_eq!(profile.function.len(), 1);
      assert_eq!(string_at(&profile, profile.function[0].name), "sampled_function");
      assert_eq!(string_at(&profile, profile.function[0].filename), "src/lib.rs");
      ...
      let thread_label = sample
        .label
        .iter()
        .find(|label| string_at(&profile, label.key) == THREAD)
        .unwrap();
      assert_eq!(string_at(&profile, thread_label.str), "worker-thread");
    }
  }
}
