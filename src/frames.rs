...
impl PartialEq for UnresolvedFrames {
  fn eq(&self, other: &Self) -> bool {
    ...
    if self.thread_id != other.thread_id || frames1.len() != frames2.len() {
      ...
      Iterator::zip(frames1.iter(), frames2.iter()).all(|(s1, s2)| s1.symbol_address() == s2.symbol_address())
    }
  }
}
...
impl Hash for UnresolvedFrames {
  fn hash<H: Hasher>(&self, state: &mut H) {
    self.frames.iter().for_each(|frame| frame.symbol_address().hash(state));
  ...
  fn from(frames: UnresolvedFrames) -> Self {
    ...
    while let Some(frame) = frame_iter.next() {
      ...
      if symbols.iter().any(|symbol| {
        ...
        matches!(&*symbol.name(), "perf_signal_handler" | "_perf_signal_handler")
      }) {
