...
impl<'a, 'b> Profiler for PProfProfiler<'a, 'b> {
  ...
  fn stop_profiling(&mut self, _benchmark_id: &str, benchmark_dir: &Path) {
    ...
    let output_file = File::create(&output_path).unwrap_or_else(|_| panic!("File system error while creating {}", output_path.display()));
    ...
    if let Some(profiler) = self.active_profiler.take() {
      match &mut self.output {
        ...
        Output::Protobuf => {
          ...
          profile.encode(&mut content).expect("Error while encoding protobuf");
          #[cfg(feature = "protobuf-codec")]
          profile.write_to_vec(&mut content).expect("Error while encoding protobuf");

          output_file.write_all(&content).expect("Error while writing protobuf");
        }
