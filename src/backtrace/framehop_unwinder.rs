use framehop::CacheNative;
use framehop::MustNotAllocateDuringUnwind;
use framehop::UnwindRegsNative;
use framehop::Unwinder;
use framehop::UnwinderNative;
use libc::c_void;
use libc::ucontext_t;
...
impl FramehopUnwinder {
  ...
  pub fn iter_frames<F: FnMut(&Frame) -> bool>(&mut self, ctx: *mut c_void, mut cb: F) {
    ...
    let mut iter = self.unwinder.iter_frames(pc, regs, &mut self.cache, &mut closure);
...
impl super::Trace for Trace {
    ...
    Self: Sized,
  {
    ...
    let Some(mut unwinder) = UNWINDER.try_write() else {
      return;
    };
