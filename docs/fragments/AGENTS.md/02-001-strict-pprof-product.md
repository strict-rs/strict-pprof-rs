...
- Keep the flat workspace layout declared in `[ecosystems.cargo.layout]` in `template.config.toml`: the root crate is the product package and `xtask/` is the local automation package.
...
Strict migration is staged. `xtask` is opted into workspace lints now; the main `pprof` crate is not opted into `[lints] workspace = true` in the current wave. Do not enable the main-crate lint opt-in opportunistically while clearing current gates; plan that as a separate user-approved wave.
