...
The `protobuf-codec` feature does not use `perftools.profiles.rs` at all: `build.rs`'s `protobuf-codegen` path regenerates fresh bindings on every build instead of reusing a committed file.
